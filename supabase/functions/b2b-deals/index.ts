...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST") {
    try {
      ...
      // ----------------------------------------------------------------
      // THE DRIVE FOLDER. Reading what Paul already drops into, instead of
      // trying to be the thing he drops onto.
      //
      // Dragging an Outlook message into a Drive-for-desktop folder is an
      // OS-level drop and Explorer handles it perfectly. Dragging into a web
      // page is a different mechanism, and the new Outlook offers nothing on it
      // but a pointer to the message on Microsoft's server -- measured
      // 2026-09-10, unfixable from a browser. So the gesture that works is left
      // alone and the folder is read instead.
      //
      // The browser NEVER talks to Apps Script. speeks.js is static and
      // readable by anyone who can load the page, so a shared secret in it is
      // not a secret; it lives here, in the function's environment, and the
      // script is unreachable without it.
      if (action === "drive_list" || action === "drive_file") {
        const base = Deno.env.get("B2B_DRIVE_URL") || "";
        const secret = Deno.env.get("B2B_DRIVE_SECRET") || "";
        if (!base || !secret) {
          return jsonResponse({
            success: false,
            error: "The Drive folder isn't connected yet. Set B2B_DRIVE_URL and "
              + "B2B_DRIVE_SECRET on this function — see google-apps-scripts/b2b-drive-proofs.gs.",
          }, 503);
        }
        ...
        if (action === "drive_list") {
          ...
          params.set("limit", String(Math.min(100, Math.max(1, Number(body.limit) || 25))));
        } else {
          params.set("action", "file");
          params.set("id", str(body.file_id, 200, "File", true)!);
        }

        // Apps Script answers a web-app GET with a 302 to googleusercontent,
        // which fetch follows by default. Worth knowing: a response that comes
        // back as HTML rather than JSON almost always means the deployment is
        // set to something other than "Anyone", and the redirect landed on a
        // Google sign-in page.
        let res: Response;
        try {
          ...
          return jsonResponse({
            success: false,
            error: `Couldn't reach the Drive folder: ${(e as Error).message}`,
          }, 502);
        }
        ...
        try {
          ...
          const looksLikeLogin = /<html/i.test(text);
          return jsonResponse({
            success: false,
            error: looksLikeLogin
              ? "The Drive script answered with a sign-in page. Its deployment needs to be "
                + '"Execute as me" and "Who has access: Anyone".'
              : "The Drive script answered with something that wasn't JSON.",
          }, 502);
        }
        if (payload.error) {
          // Passed through as written: these are the script's own words about
          // its own configuration, and rewording them loses the diagnosis.
          return jsonResponse({ success: false, error: String(payload.error) }, 502);
        }

        if (action === "drive_list") {
          return jsonResponse({ success: true, files: payload.files ?? [] });
        }

        // Handed back as a data URI so the client can turn it into a File and
        // send it through add_proof like a dropped or pasted message. The bytes
        // make a round trip they could avoid, and that is the point: one path
        // parses the headers, labels the row and inserts it, so a Drive attach
        // cannot drift away from a dropped one. This feature's whole history is
        // bugs from having several routes that were supposed to agree.
        const mime = String(payload.mime || "");
        const data = String(payload.data || "");
        if (!data) return jsonResponse({ success: false, error: "That file came back empty." }, 502);
        // base64 is 4 chars per 3 bytes, so this bounds the decoded size
        // without decoding it here.
        if ((data.length / 4) * 3 > PROOF_MAX_BYTES) {
          return jsonResponse({
            success: false,
            error: `That file is over the ${Math.round(PROOF_MAX_BYTES / 1e6)}MB limit.`,
          }, 400);
        }
        return jsonResponse({
          success: true,
          name: String(payload.name || "message"),
          mime,
          bytes: Number(payload.bytes || 0),
          data_uri: `data:${mime || "application/octet-stream"};base64,${data}`,
        });
      }
